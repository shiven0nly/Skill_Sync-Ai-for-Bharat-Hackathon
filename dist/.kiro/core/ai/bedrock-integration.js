...
`;
    ...
    async batchAnalyze(requests) {
        const results = await Promise.allSettled(requests.map(request => this.generateExplanation(request)));
 ...
 */
export function createBedrockAIService(region = 'us-east-1') {
    const config = {
        region,
...
//# sourceMappingURL=bedrock-integration.js.map