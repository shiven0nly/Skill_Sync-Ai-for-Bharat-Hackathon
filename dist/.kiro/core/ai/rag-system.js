...
export class RAGSystem {
    constructor(bedrockService, config = {}) {
     ...
     */
    async semanticSearch(queryEmbedding, language) {
        const results = [];
    ...
    applyDiversityFiltering(results) {
        if (results.length <= 3)
            return results;
        const diverse = [results[0]]; // Always include top result
        ...
        for (let i = 1; i < results.length; i++) {
            ...
            for (const selected of diverse) {
                const contentSimilarity = this.textSimilarity(candidate.entry.content, selected.entry.content);
     ...
     */
    calculateRelevanceScore(entry, similarity) {
 ...
 */
export function createRAGSystem(bedrockService, config) {
...
export const RAGUtils = {
     ...
     */
    estimateMemoryUsage(entryCount, embeddingDimensions = 1536) {
     ...
     */
    generateQuerySuggestions(code, language) {
        const suggestions = [
