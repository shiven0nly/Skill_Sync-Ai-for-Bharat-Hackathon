 ...
 */
import { extractConditionals, extractLoops, extractImports, getLineCount, } from './parserUtils';
/**
 * Calculate cyclomatic complexity from parsed code
 * Counts decision paths: conditionals, loops, and logical operators
 */
export function calculateCyclomaticComplexity(rootNode, language) {
 ...
 */
export function calculateDependencyDepth(rootNode, language) {
 ...
 */
export function calculateComplexity(rootNode, fileContent, userSkillLevel, language = 'javascript') {
    ...
    // C_L = ((V_c × 0.5) + (D_d × 0.3) + (log₁₀(LOC) × 0.2)) / S_level
    const numerator = cyclomaticComplexity * 0.5 +
        dependencyDepth * 0.3 +
    ...
    const verdict = complexity === 'high'
        ? 'High Load 🟥'
        : complexity === 'medium'
            ? 'Medium Load 🟨'
            : 'Low Load 🟢';
    const suggestion = complexity === 'high'
 ...
 */
export const analyzeCode = (fileContent, userSkill) => {
    const lines = fileContent.split('\n').length;
    // Simplified logic for a hackathon demo:
    const cyclomaticComplexity = (fileContent.match(/if|for|while|map|&&|\|\|/g) || []).length;
    const dependencyDepth = (fileContent.match(/import|require/g) || []).length;
    // The Kiro Logic: $C_L$ Formula
    const cognitiveLoad = (cyclomaticComplexity * 0.5 +
        dependencyDepth * 0.3 +
        Math.log10(lines) * 0.2) /
        userSkill;
    return {
        score: Math.min(100, Math.round(cognitiveLoad * 10)),
        verdict: cognitiveLoad > 7
            ? 'High Load 🟥'
            : cognitiveLoad > 4
                ? 'Medium Load 🟨'
                : 'Low Load 🟢',
        suggestion: cognitiveLoad > 7 ? 'Needs Bridge Explanation' : 'Read as is',
    };
};
