...
import { extractFunctions, extractImports, extractConditionals, extractLoops, calculateNestingDepth, getLineCount, parseCode, } from './parserUtils';
/**
 * Example: Analyze a JavaScript file
 */
async function analyzeJavaScriptFile(filePath, code) {
    ...
    const cognitiveLoad = ((cyclomaticComplexity * 0.5) + (dependencyDepth * 0.3) + (Math.log10(lineCount) * 0.2)) /
        (userSkill / 5);
    return {
        filePath,
        language,
 ...
 */
async function analyzePythonFile(filePath, code) {
    const manager = await getParserManager();
    const language = detectLanguage(filePath, code);
    if (!language) {
        throw new Error(`Could not detect language for ${filePath}`);
 ...
 */
async function analyzeGoFile(filePath, code) {
    const manager = await getParserManager();
    const language = detectLanguage(filePath, code);
    if (!language) {
        throw new Error(`Could not detect language for ${filePath}`);
