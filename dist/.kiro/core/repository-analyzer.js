...
import { parseCode, extractFunctions, extractImports, extractConditionals, extractLoops, getLineCount } from './analysis/parserUtils';
/**
 ...
 */
async function analyzeFile(filePath, skillLevel, parserManager) {
    try {
        // Read file content
 ...
 */
function calculateCyclomaticComplexity(conditionals, loops, functions) {
 ...
 */
function calculateDependencyDepth(imports) {
 ...
 */
function filterFiles(files, excludePatterns, includePatterns) {
    const allExcludePatterns = [...DEFAULT_EXCLUDE_PATTERNS, ...excludePatterns];
    let filteredFiles = files;
    // Apply include patterns if specified
    if (includePatterns && includePatterns.length > 0) {
        filteredFiles = files.filter(file => includePatterns.some(pattern => matchesPattern(file, pattern)));
    }
    // Apply exclude patterns
    filteredFiles = filteredFiles.filter(file => !allExcludePatterns.some(pattern => matchesPattern(file, pattern)));
