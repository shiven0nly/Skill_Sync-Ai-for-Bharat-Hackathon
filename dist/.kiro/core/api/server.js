...
import { calculateComplexity, analyzeCode } from '../analysis/complexityAnalyzer';
 ...
 */
function findFiles(dirPath, config) {
    const files = [];
        ...
        try {
            ...
            for (const item of items) {
                ...
                if (stat.isDirectory()) {
                    ...
                    const shouldSkip = config.excludePatterns.some(pattern => item.includes(pattern) || fullPath.includes(pattern));
                    if (!shouldSkip) {
                        traverse(fullPath);
                    }
                }
 ...
 */
async function analyzeFileLegacy(filePath, config) {
    try {
 ...
 */
async function analyzeFile(filePath, config, parsers) {
    try {
        const content = readFileSync(filePath, 'utf-8');
        const language = detectLanguageFromContent(content);
        ...
        const metrics = calculateComplexity(rootNode, content, config.userSkillLevel, language);
        return {
            path: filePath,
            language,
            metrics,
 ...
 */
export async function generateComplexityMap(repositoryPath, config = {}) {
...
export class ComplexityApiServer {
     ...
     */
    async handleRequest(url, method, body) {
     ...
     */
    async processRequest(method, url, body) {
        try {
 ...
 */
export async function analyzeRepositoryCLI(repositoryPath, userSkillLevel = 5) {
