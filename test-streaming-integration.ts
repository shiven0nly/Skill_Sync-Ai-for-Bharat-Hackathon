...
import { startApiServer, SkillSyncApiServer } from './.kiro/core/api-server.js';
 ...
 */
async function testStreamingBridge(): Promise<TestResult> {
  let server: SkillSyncApiServer | null = null;
 ...
 */
function parseSSEEvent(eventText: string): { eventType: string; data: any } {
 ...
 */
function validateStreamingEvents(events: Array<{ eventType: string; data: any }>): {
  valid: boolean;
  issues: string[];
} {
  const issues: string[] = [];
