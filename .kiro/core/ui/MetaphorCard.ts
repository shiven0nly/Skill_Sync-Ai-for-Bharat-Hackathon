...
export class MetaphorCardGenerator {
  private config: MetaphorGenerationConfig;
   ...
   */
  private analyzeCodePatterns(codeSnippet: string, language: string): string[] {
  ...
  private selectMetaphor(
    ...
    const template = metaphorTemplates[primaryPattern as keyof typeof metaphorTemplates] || metaphorTemplates['conditional'];

    // Adjust difficulty level based on complexity and user skill
    const difficultyLevel = Math.min(10, Math.max(1, 
      Math.round(complexity.cognitiveLoad * 2) - userSkillLevel + 5
    ));
  ...
  static generateRepositorySummary(metaphorCards: MetaphorCardData[]): {
    ...
    const averageDifficulty = metaphorCards.reduce(
      (sum, card) => sum + card.metaphor.difficultyLevel, 0
    ) / totalCards;
