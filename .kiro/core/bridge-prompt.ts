...
interface MentalModelMapping {
  ...
  mentalModel: string;
  description: string;
  skillLevelThreshold: number; // Minimum skill level to show this mapping
}
 ...
 */
const MENTAL_MODEL_MAPPINGS: Record<SupportedLanguage, MentalModelMapping[]> = {
  javascript: [
    {
      construct: 'useEffect',
 ...
 */
export function generateFileBridgeExplanation(
 ...
 */
export function generateRepositoryBridgeExplanation(
  ...
  const hotspots = hotspotFiles.map(file => 
    generateFileBridgeExplanation(file, skillLevel)
  );
 ...
 */
function getMentalModelsForSkillLevel(
  language: SupportedLanguage,
  skillLevel: number
): MentalModelMapping[] {
  const languageMappings = MENTAL_MODEL_MAPPINGS[language] || [];
  
  return languageMappings.filter(mapping => 
    mapping.skillLevelThreshold <= skillLevel
  );
}
 ...
 */
function generateMetaphor(cognitiveLoad: number, skillLevel: number): string {
  const metaphors = {
    low: [
  ...
  let category: keyof typeof metaphors;
  if (cognitiveLoad <= 1) category = 'low';
  else if (cognitiveLoad <= 2) category = 'medium';
  else if (cognitiveLoad <= 3) category = 'high';
...
function generateComplexityExplanation(
  complexity: any,
  skillLevel: number
): { level: string; reasoning: string; suggestions: string[] } {
